 ...
 * Cyclopses age, move, eat humans, and die.
 *
 * @author Zahra Amaan and Imaan Ghafur
 */
public class Cyclops extends Animal
{
    //probability of the cyclops getting the disease
    public double diseaseProb = 0.02;
    private Time t;
    // maximum litter size
    protected static final int MAX_LITTER_SIZE = 3;
     /**
     * Create a cyclops. A cyclops can be created as a new born (age zero
     ...
     */
    public Cyclops(boolean randomAge, Field field, Location location, Time time)
    {
    ...
    /**
     * This is what the cyclops does most of the time: it hunts for
    ...
    public void act(List<Living> newAnimals)
    {
        ...
        if(isAlive()) {
            ...
            if(t.isWinter && Math.random()<=deathProbability){ // freeze to death
                setDead();
    ...
    /**
     * returns the probability of cyclops getting the disease
     */
    ...
    {
        // New cyclopses are born into adjacent locations.
        ...
        if (isFemale()){
            ...
            for(int b = 0; b < births && free.size() > 0; b++) {
                ...
                Animal young = new Cyclops(false, field, loc,t);
    ...
    private boolean canBreed()
    {
        ...
        while(it.hasNext()) {
            Location where = it.next();
            Object animal = field.getObjectAt(where);
            if((age >= BREEDING_AGE) && animal instanceof Cyclops) {
                Cyclops c = (Cyclops)animal;
                if(!c.isFemale()){
                    return true;
                }
            }
